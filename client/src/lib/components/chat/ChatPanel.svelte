    ...
    {#if !embedded}<header
      ...
      >
        <div class="flex items-center gap-2 text-cyan-300">
          ...
          <h2 class="m-0 text-sm text-slate-200">Room chat</h2>
        </div>
        <button
          class="grid min-h-8 min-w-8 place-items-center rounded-md bg-transparent text-slate-400 hover:bg-slate-800 hover:text-slate-50 focus-visible:bg-slate-800 focus-visible:text-slate-50 focus-visible:outline-none"
          type="button"
          onclick={() => onOpenChange(false)}
          aria-label="Close chat"
          ><Icon icon="mdi:close" class="size-5" /></button
        >
      </header>{/if}
