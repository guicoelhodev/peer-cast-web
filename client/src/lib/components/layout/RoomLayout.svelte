...
>
  ...
  >
    <div class="brand flex h-12 items-center border-b border-slate-800 px-4">
      <strong class="text-sm tracking-[.04em]">PeerCast</strong>
    </div>
    <div
      class="sidebar-content flex min-h-0 flex-1 flex-col gap-4 overflow-hidden p-3"
    >
      <div>
        <p
          class="side-label mb-2 text-[.625rem] font-bold uppercase tracking-[.14em] text-slate-500"
        >
          Connection
        </p>
        <div class="connection grid gap-1.5 rounded-lg bg-slate-800/58 p-2.5">
          <span class="status font-mono text-[.7rem] text-emerald-300"
            >● {status}</span
          ><span class="room-id break-all font-mono text-[.62rem] text-cyan-300"
            >ROOM / {roomId}</span
          >
        </div>
      </div>
      <div class="shrink-0">
        ...
        <ul class="m-0 max-h-32 list-none overflow-y-auto p-0">
          <li
            class="flex min-w-0 items-center gap-1.5 border-b border-slate-800/70 py-2 text-xs text-slate-300"
          >
            <i class="local-dot h-1.5 w-1.5 flex-none rounded-full bg-cyan-400"
            ></i><span class="overflow-hidden text-ellipsis whitespace-nowrap"
              >{local.displayName} (you)</span
            >{#if local.isHost}<em
                class="ml-auto text-[.62rem] not-italic text-emerald-300"
                >Host</em
              >{/if}
          </li>
          {#each participants as participant (participant.id)}<li
              class="flex min-w-0 items-center gap-1.5 border-b border-slate-800/70 py-2 text-xs text-slate-300"
            >
              <i class="h-1.5 w-1.5 flex-none rounded-full bg-purple-500"
              ></i><span class="overflow-hidden text-ellipsis whitespace-nowrap"
                >{participant.displayName}</span
              >{#if participant.isHost}<em
                  class="ml-auto text-[.62rem] not-italic text-emerald-300"
                  >Host</em
                >{/if}
            </li>{/each}
  ...
  <div class="room-main flex min-h-[75dvh] min-w-0 flex-col lg:min-h-0">
    ...
    >
      <CallGrid {local} {participants} />{#if error}<p
          class="error mt-3 mb-0 text-xs text-red-300"
          role="alert"
        >
          {error}
        </p>{/if}
      <div class="stage-controls mt-3 flex justify-center">
        <CallControls
          mode="devices"
          {microphoneMuted}
          {videoState}
