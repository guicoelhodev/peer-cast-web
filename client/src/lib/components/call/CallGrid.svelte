<script lang="ts">
	...
	let tiles = $derived([{ ...local, id: local.id || 'local' }, ...participants.filter((participant) => participant.id !== local.id)]);
	let overflowCount = $derived(tiles.length > 9 ? tiles.length - 8 : 0);
	let gridTiles = $derived(overflowCount > 0 ? tiles.slice(0, 8) : tiles);
	let visibleTiles = $derived(focusedId ? gridTiles.filter((participant) => participant.id === focusedId) : gridTiles);
	let slotCount = $derived(focusedId ? 1 : gridTiles.length + (overflowCount > 0 ? 1 : 0));
...
<section class:focused={Boolean(focusedId)} class={`call-grid count-${slotCount} grid min-h-0 w-full min-w-0 flex-1 gap-3 max-md:!grid-cols-1 max-md:!grid-rows-none max-md:auto-rows-[minmax(13rem,1fr)] max-md:content-start max-md:overflow-y-auto ${layoutClass}`} aria-label="Call participants">
	...
	{#if overflowCount > 0 && !focusedId}
		<div class="tile grid min-h-0 min-w-0 place-items-center overflow-hidden rounded-xl border border-slate-800 bg-slate-900 text-slate-300 max-md:min-h-52" role="status" aria-label={`${overflowCount} more participants`}>
			<div class="flex flex-col items-center gap-2 text-center">
				<div class="grid size-14 place-items-center rounded-full bg-cyan-400/10 text-cyan-300"><Icon icon="mdi:account-multiple" class="size-7" /></div>
				<strong class="text-2xl font-semibold text-slate-100">+{overflowCount}</strong>
